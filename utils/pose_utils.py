...
def render_representation(cvimage, repr, withids, color = None):
    ...
    for i in range(26):
        position = (int(positions[i][0] * size[0]), int(positions[i][1]*size[1]))
        cv2.circle(cvimage, position, 3,  color, -1)
        if withids:
            cv2.putText(cvimage, str(i), position, cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1, cv2.LINE_AA)
    return cvimage
